...
//validation username
function checkUsername(&$username, &$erroreU){  //&$nome, &$cognome, 
	...
	//strcasecmp --> confronto tra stringhe NO CASE SENSITIVE / strcasecmp($username,$nome)==0 || strcasecmp($username,$cognome)==0 || strcasecmp($username,$nc)==0 || strcasecmp($username,$cn)==0
	if(!preg_match("/^([\w]{4,})*$/",$username)){	//controllo se NON rispetta le condizioni: avere almeno quattro lettera
		$erroreU = "Inserire un username valido (almeno 4 caratteri)";	//modifico valore dell'errore / L'username non può contenere il tuo nome o cognome
	}
}

//validation password - controllo correttezza
function checkPassword(&$password, &$erroreP) {
	if(empty($password)){	//controllo che non sia vuoto
		$erroreP = "Inserire la password";	//modifico valore dell'errore
		return 0;	//esce dalla funzione altrimenti entrerebbe nel secondo if che ha una condizione simile
	}
	if(!preg_match("/^(?=.*\d)(?=.*[@#\-_$%^&+=§!\?])(?=.*[a-z])(?=.*[A-Z])[0-9A-Za-z@#\-_$%^&+=§!\?]{8,20}+$/",$password)){	//controllo se NON rispetta le condizioni: avere almeno una lettera minuscola, una maiuscola, un carattere speciale (tra quelli accettati) e un numero e che poi sia composta da 8 a 20 caratteri
...
//validation password - controllo che quella di conferma sia uguale a quella nuova
function checkPsw(&$psw, &$password, &$errorePsw) {
	if(empty($psw)){	//controllo che non sia vuoto
		$errorePsw = "Inserire la password";	//modifico valore dell'errore
		return 0;	//esce dalla funzione altrimenti entrerebbe nel secondo if che ha una condizione simile
	}
	if(strcmp($password,$psw)!=0){	//controllo se NON rispetta le condizioni: uguaglianza tra le due password
		$errorePsw = "Inserire la password corretta";	//modifico valore dell'errore
	}
}

//validation password - controllo che quella nuova sia diversa da quella vecchia
function check_OldPsw(&$password, &$erroreP) {
	if(strcmp($password,$_SESSION['password'])==0){	//controllo se rispetta le condizioni: uguaglianza tra la vecchia e la nuova password
		$erroreP = "Inserire una password diversa da quella vecchia";	//modifico valore dell'errore
	}
}

//validation checkbox privacy
function checkPrivacy(&$privacy,&$errorePr) {
	if(strcmp($privacy,"privacy")!=0){	//controllo che non sia checkato tramite la stringa "privacy" impostata nel form dal value
		$errorePr = "Accettare la noramtiva";	//modifico valore dell'errore
	}
}
?>
